...
unsigned long measure_rc_time() {
  ...
  // Wait for full discharge (>10τ for wettest case: 10 * 1ms = 10ms)
  delay(10);
  ...
  // Wait for pin to go HIGH or timeout after 1 second
  while (digitalRead(WATER_SENSE_PIN_B) == LOW) {
    if (micros() - start > 1000000) {
      // Timeout - extremely high resistance (open circuit or very dry)
      return 1000000;
    }
  }
